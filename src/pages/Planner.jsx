...
const Planner = () => {
    ...
    const [formData, setFormData] = useState({ city: cityParam, days: 3, preferences: '' });
    ...
    const handleGenerate = async (e) => {
        ...
        try {
            ...
            const itineraryReq = axios.post('https://smart-travel-ai-backend.onrender.com/api/.../itinerary/generate', formData);
            // We trigger these but don't block display if itinerary is ready first
            const packingReq = axios.post('https://smart-travel-ai-backend.onrender.com/api/.../itinerary/packing', {
                city: formData.city, season: 'Spring', activities: formData.preferences
            });
            const etiquetteReq = axios.post('https://smart-travel-ai-backend.onrender.com/api/.../itinerary/culture', { city: formData.city });
    ...
    const handleSaveTrip = async () => {
        ...
        setSaving(true);
        try {
            await axios.post('https://smart-travel-ai-backend.onrender.com/api/.../trips/save', {
                city: formData.city,
